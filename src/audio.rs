...
impl AliasingParams {
    pub fn from_secs(target_duration_secs: f32, factor: usize) -> Self {
        Self {
            factor,
...
pub fn export_file(from: &String, to: &String, params: &AliasingParams) -> Receiver<()> {
    ...
    thread::spawn(move || {
        ...
        let mut aliased = libaaarg::alias(samples, params.target_duration, params.factor);
...
pub fn preview_aliasing(selected_file: &String, params: &AliasingParams, sink: Arc<Mutex<Sink>>) {
    ...
    thread::spawn(move || {
        ...
        let buf = libaaarg::alias(samples, params.target_duration, params.factor);
